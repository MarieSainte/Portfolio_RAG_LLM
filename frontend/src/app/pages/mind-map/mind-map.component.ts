    ...
    <div class="page-enter mind-map-page">
        ...
        >
          ...
          >
            ...
            @for (n of nodes; track n.data.id; let i = $index) {
              ...
              >
                ...
                <span class="mm-node__label">{{ 'MIND_MAP.' + n.data.id + '.SHORT' | translate }}</span>
              </button>
            }
          ...
          <div class="mm-controls" (pointerdown)="$event.stopPropagation()">
            <button type="button" (click)="zoomBy(1.25)" [attr.aria-label]="'MIND_MAP.ZOOM_IN' | translate">+</button>
            <button type="button" (click)="zoomBy(0.8)" [attr.aria-label]="'MIND_MAP.ZOOM_OUT' | translate">−</button>
            <button type="button" (click)="fit()" [attr.aria-label]="'MIND_MAP.RESET' | translate">⌂</button>
          </div>
